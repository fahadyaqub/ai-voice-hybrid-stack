...
for _name, server in servers.items():
    ...
    # Rewrite filesystem mounts for OpenClaw container paths.
    ...
    # Rewrite skill MCP adapter paths for OpenClaw container paths.
...
install_skills_runtime_deps() {
  ...
  echo "==> [local] installing skill MCP runtime dependencies (best effort)"
